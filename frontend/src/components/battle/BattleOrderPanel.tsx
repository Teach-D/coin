// ============================================================================
// CUSTOMIZATION — 브랜드 컬러만 이 구역에서 수정
// ============================================================================
...
import { useBattleBuyOrder } from '../../hooks/useBattleOrder';

interface BattleOrderPanelProps {
  battleId: string;
  ticker: string;
  currentPrice: number;
...
export function BattleOrderPanel({ battleId, ticker, currentPrice, battleBalance }: BattleOrderPanelProps) {
  ...
  const buyOrder = useBattleBuyOrder(battleId);
      ...
      <div>
        <div className="flex justify-between items-center mb-1.5">
          ...
          <span className="text-xs text-zinc-500">
            배틀 잔고{' '}
            <span className="font-semibold text-zinc-300">{formatKRW(battleBalance)} KRW</span>
          </span>
        </div>
        <div className={`flex items-center rounded-xl border px-3 py-2.5 transition-colors bg-zinc-800 border-zinc-700 ${dirColors.inputBorder}`}>
          <input
